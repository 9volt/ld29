...
public class CameraMove : MonoBehaviour {
	...
	void Update () {
		...
		if(Input.mousePosition.x > Screen.width * .95f && camera.ScreenToWorldPoint(new Vector3(Screen.width, 0, 0)).x < (wg.transform.position.x + wg.width - 1) && Input.mousePosition.x < Screen.width * .99f){
			//scroll right
		...
		if(Input.mousePosition.x < Screen.width * .05f && camera.ScreenToWorldPoint(new Vector3(0, 0, 0)).x > wg.transform.position.x && Input.mousePosition.x > Screen.width * .01f){
			//scroll left
			transform.Translate(Vector3.left * scroll_speed);
			bg.renderer.material.mainTextureOffset = new Vector2( (bg.renderer.material.mainTextureOffset.x + -bg_scroll_speed)%1, 0f);
		}
		if(Input.mousePosition.y > Screen.height * .95f && camera.ScreenToWorldPoint(new Vector3(0, 0, 0)).y > wg.transform.position.y && Input.mousePosition.y < Screen.height * .99f){
			//scroll up
			transform.Translate(Vector3.up * scroll_speed * Time.deltaTime);
		}
		if(Input.mousePosition.y < Screen.height * .05f && camera.ScreenToWorldPoint(new Vector3(0, Screen.height, 0)).y < (wg.transform.position.y - wg.height && Input.mousePosition.y > Screen.height * .01f)){
			//scroll down
