...
public class CameraMove : MonoBehaviour {
	...
	void Update () {
		...
		if((Input.mousePosition.x > Screen.width * .9f || Input.GetAxis("Horizontal") > 0f) && camera.ScreenToWorldPoint(new Vector3(Screen.width, 0, 0)).x < (wg.transform.position.x + wg.width - 2)){
			//scroll right
		...
		if((Input.mousePosition.x < Screen.width * .1f || Input.GetAxis("Horizontal") < 0f) && camera.ScreenToWorldPoint(new Vector3(0, 0, 0)).x > wg.transform.position.x + 1){
			//scroll left
			transform.Translate(Vector3.left * scroll_speed * Time.deltaTime);
			bg.renderer.material.mainTextureOffset = new Vector2( (bg.renderer.material.mainTextureOffset.x + -bg_scroll_speed)%1, 0f);
		}
		if((Input.mousePosition.y > Screen.height * .9f || Input.GetAxis("Vertical") > 0f) && camera.ScreenToWorldPoint(new Vector3(0, Screen.height, 0)).y < wg.transform.position.y - 2){
			//scroll up
			transform.Translate(Vector3.up * scroll_speed * Time.deltaTime);
		}
		if((Input.mousePosition.y < Screen.height * .1f || Input.GetAxis("Vertical") < 0f) && camera.ScreenToWorldPoint(new Vector3(0, 0, 0)).y > (wg.transform.position.y - wg.height + 2)){
			//scroll down
